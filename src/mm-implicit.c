...
static void coalesce() {
    for (block_t *curr = mm_heap_first; mm_heap_last != NULL && curr < mm_heap_last;
        ...
        block_t *next = (void *) curr + curr_size;
        if (!is_allocated(curr) && !is_allocated(next)) {
            size_t next_size = get_size(next);
            set_header(curr, curr_size + next_size, false);
...
void *mm_realloc(void *old_ptr, size_t size) {
    ...
    if (size <= old_size - sizeof(block_t)) {
        return old_ptr;
    }
